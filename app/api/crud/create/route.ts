...
const getTablePageTemplate = (apiEndpoint: string, filters: FormInput[], tableHeader: string) => `
...
import ${capitalizeFirstChar(
	rootFolderName.split("/")[rootFolderName.split("/").length - 1],
)}FormPage from '@/components/pages/_${rootFolderName}/${rootFolderName.split("/")[rootFolderName.split("/").length - 1]
	}Form.page';
 import { fetchHelper } from '@/api/fetch';
...
async function createFolderStructure({
	...
	const files = [
		...
		{
			path: path.join(rootPath, `${capitalizeFirstChar(tableHeader)}Columns.tsx`),
	...
	if (typeFileContent) {
		files.push({
			path: path.join(rootPath, "types.ts"),
			content: typeFileContent,
		});
	}
