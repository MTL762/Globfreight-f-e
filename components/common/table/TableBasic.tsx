...
export default function TableBasic<T extends Record<string, unknown>>({
  data,
  columns,
  tableActions,
  filters,
                  ...
                  {tableActions && Object.values(tableActions).filter(Boolean)?.length > 0 && (
                    <TableHead
                      className={cn(
                        ...
                        tableActions?.fixedActions &&
                          "sticky end-0 bg-gray-50 dark:bg-[#020817] z-1 shadow-[-4px_0_4px_-2px_rgba(0,0,0,0.05)] rtl:shadow-[4px_0_4px_-2px_rgba(0,0,0,0.05)]",
                        tableActions?.fixedActions &&
                          isInnerTable &&
                      ...
                      {tableActions && Object.values(tableActions).filter(Boolean)?.length > 0 && (
                        <TableCell
                          className={cn(
                            ...
                            tableActions?.fixedActions &&
                              "sticky end-0 z-1 shadow-[-4px_0_4px_-2px_rgba(0,0,0,0.05)] rtl:shadow-[4px_0_4px_-2px_rgba(0,0,0,0.05)]",
                            tableActions?.fixedActions &&
                              (isInnerTable
                                ? "bg-gray-50/50 dark:bg-slate-900"
                                : "bg-white dark:bg-[#020817]"),
                            tableActions?.fixedActions &&
                              "group-hover:bg-gray-50 dark:group-hover:bg-slate-900"
                          )}
                        >
                          <TableActions
                        ...
                        <TableRow className="bg-gray-50/50 dark:bg-slate-900/30 border-b border-gray-100 dark:border-gray-800">
                          <TableCell
                            colSpan={
                              ...
                              (tableActions &&
                              Object.values(tableActions).filter(Boolean)?.length > 0
                                ? 1
                                : 0) +
                              1
                            }
